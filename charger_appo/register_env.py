"""
Charger APPO Environment Registration
"""
import numpy as np
...
class MultiEnv(gymnasium.Wrapper):
    """
    Wrapper for managing multiple parallel environments.
    Aggregates observations, rewards, and done signals across all environments.
    """
    ...
    def __init__(self, env_cfg: Environment, preprocessing_cfg: PreprocessorConfig):
        if env_cfg.target_num_agents is None:
            ...
            assert env_cfg.target_num_agents % env_cfg.grid_config.num_agents == 0, \
                "Target num_agents must be divisible by num_agents"
    ...
    def step(self, actions):
        """Step all environments and aggregate results."""
    ...
    def sample_actions(self):
        """Sample random actions from all environments."""
        actions = []
        for env in self.envs:
...
class CustomEnv:
    """Custom environment factory for Sample Factory."""
