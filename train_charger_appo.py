...
def main():
    ...
    print("="*60)
    print(f"Train dir: {experiment.get('train_dir')}")
    print(f"Train steps: {experiment.get('train_for_env_steps')}")
    print(f"\nReward Configuration:")
    preprocessing_config = experiment.get('preprocessing')
    print(f"  Intrinsic target reward: {preprocessing_config.get('intrinsic_target_reward')}")
    print(f"  On target reward: {preprocessing_config.get('on_target_reward')}")
    print(f"  Battery reward coeff: {preprocessing_config.get('battery_reward_coeff')}")
    print(f"  Battery reward type: {preprocessing_config.get('battery_reward_type')}")
    print(f"  Min charge level: {preprocessing_config.get('min_charge_level')}")
    print(f"  Earlydeath penalty coeff: {preprocessing_config.get('early_death_penalty_coeff')}")
